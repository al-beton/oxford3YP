...
#define PID_RANGE 100  //must be even
...
const byte median_filter_size = 7;
...
void loop() {
  ...
  alt_cm = alt_cm * (4.0/5.0) + sensor.getDist()* (1.0/5.0);
  ...
  if (time_elapsed >= 200) { //5Hz data feed
    last_time = last_time + time_elapsed;
    // Print out variables for video data graphics
    Serial.print(alt_cm);
    Serial.print("\t");
    Serial.print(rx_throttle);
    Serial.print("\t");
