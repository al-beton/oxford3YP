...
draw = 1;       %graph toggle
simulations = 1;       %number of simulations (keep this low if draw = 1)
...
fish_density = (1 * tip_accuracy / fraction_fish_visible_air) / (0.5 * 2 * 5 * (1/0.239957)); %0.539957% estimated density of "fish tips" using the mean average distance between tips

%drones
drone_max_flight_time = 0.5;      % 2 hours
drone_scan_time = 1 / 60;       % 1min / 60min/hr
number_of_drones = 4;
...
fish_field_type = 'scatter';

% Calculate max number of scans for given battery life
scans = calculate_scans(number_of_drones, distance_between_scans, drone_search_pattern, centre_scan, drone_max_flight_time, drone_speed, drone_scan_time, ship_location);

% Generate drones
drones = generate_drones(number_of_drones, distance_between_scans, scans, drone_search_pattern, centre_scan);
...
for i = [1:simulations]
    ...
    if draw == 1
        ...
        title('Fishing Aid Model Simulation')
        text(-2.3,5.5,'5km Reference Radius','Color','red','FontSize',8)
        xlabel('x (km)') 
        ylabel('y (km)') 
        axis([(-1.5 * marker_radius) (1.5 * marker_radius) (-1.5 * marker_radius) (1.5 * marker_radius)]);  %set boundaries of plot
        %plot ship
        plot(0, 0, 'rd');
        ...
        if demonstation_step == 1 pause, end
        %plot scan radius
        viscircles([0, 0], marker_radius, 'Color', 'r', 'LineWidth', 0.25);
        if demonstation_step == 1 pause, end
        %plot drone routes
...
no_drone_cost = ((5 / 0.539957) / tip_accuracy) * ship_cost_per_distance;
no_drone_time = ((5 / 0.539957) / tip_accuracy) / ship_speed;
disp([' ']);    %white space
...
%calculate max number of scans for given scan pattern and max flight time
function scans = calculate_scans(number_of_drones, distance_between_scans, drone_search_pattern, centre_scan, drone_max_flight_time, drone_speed, drone_scan_time, ship_location)

centre_scan_shortest = (number_of_drones == 1) && (centre_scan == 1);
...
while time <= drone_max_flight_time
    scans = scans + scan_step;
    route = generate_route(1, 1, distance_between_scans, scans, drone_search_pattern, centre_scan_shortest);
    distance = trip_length(ship_location, route, ship_location);
    time = distance/drone_speed + scans * drone_scan_time;
    
end
scans = scans - scan_step;
    
end
%generate specified route
function r = generate_route(drone_i, number, distance_between_scans, scans, drone_search_pattern, centre_scan)
    if strcmp(drone_search_pattern, 'triangle')
        r = triangle_route(drone_i, number, distance_between_scans, scans, centre_scan);
    else if strcmp(drone_search_pattern, 'spiral')
            r = spiral_route(drone_i, number, distance_between_scans, scans, centre_scan);
        end
    end
end

%generate triangular drone route
function tr = triangle_route(drone_i, number, distance_between_scans, scans, centre_scan)
...
scans_each_way = floor(scans / 2);

out_route = exp(1i * theta) * [1 + out_offset : scans_each_way + out_offset] * distance_between_scans;
in_route = exp(1i * (theta + d_theta)) * flip([1:scans_each_way]) * distance_between_scans;
tr = [out_route, in_route];

end

%generate spiral drone route
function sr = spiral_route(drone_i, number, distance_between_scans, scans, centre_scan)
...
function drones = generate_drones(number, distance_between_scans, scans, drone_search_pattern, centre_scan)
drones = [];
for i = 1:number
    drones = [drones, Drone(0, generate_route(i, number, distance_between_scans, scans, drone_search_pattern, centre_scan))];
end
end
